...
Read only the requested mode.

## Daily

Use for today, yesterday, or one specific date.

- Query all running activities for the date, then inspect the primary activity at lap level.
- Show total distance, time, average pace, average heart rate, training load, and current recovery.
- Identify the workout structure and its execution quality.
- Include one main conclusion, strengths, current concern, recent load context, and the next scheduled workout.
- If multiple runs exist, distinguish the primary session from warm-up, cool-down, commute, or recovery work.

## Weekly

Use the user's local calendar week unless another range is stated.
...
Use the user's local calendar month unless another range is stated.
